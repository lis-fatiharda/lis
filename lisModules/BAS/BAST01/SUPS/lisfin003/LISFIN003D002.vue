  ...
  <l-div v-if="mv.lisDialog == 'LISFIN003D002'">
    ...
    <l-card>
      ...
      <q-bar :class="`bg-${tabInfo.moduleColor}`">
        <l-toolbar-title>{{
          this.$gl("Muhasebe Belgeleri", "Accounting Documents")
        }}</l-toolbar-title>
        <l-btn
          v-if="mv.modi != 2"
          icon="save"
    ...
    <l-card>
      <l-card-section horizontal>
        <l-div-flex>
          ...
          ></l-input>
          <l-select
            :label="this.$gl(`İmzalama Protokolü`, `Sign Protocol`)"
            map-options
            v-model="mv.lisfin003.signprotocol"
            :options="[
              { value: 0, label: this.$gl(`İmza Gerektirmez`, `No Sign`) },
              { value: 1, label: this.$gl(`A veya B İmzalar`, `Sign A or B`) },
              {
                value: 2,
                label: this.$gl(
                  `B ve A Sırayla İmzalar`,
                  `B and A Signatures in Order`
                ),
              },
              {
                value: 3,
                label: this.$gl(`Yalnız A İmzalar`, `Only A Signatures`),
              },
            ]"
            optValue="value"
            optTitle="label"

    ...
    methods: {
        ...
        async searchOpenItems() {
            this.openitems = await this.lis.function(
                "SALT01/04-searchOpenItems",
                this.dv
            );
            // Clear exits openitems in the document
            for (let i in this.openitems) {
                let isexist = this.dv.lissaldocs.items.filter(
                    (e) =>
        ...
        async copyFromOpenItems(pItem, pIndex) {
            for (let i in this.openitems) {
                if (this.openitems[i]._selected == true) {
                    this.openitems[i].itemnum =
                        parseInt(
                            this.dv.lissaldocs.items[
                                this.dv.lissaldocs.items.length - 1
                            ].itemnum
                        ) + 10;
                    if (this.dv.lissaldocs.items[0].material == "") {
                        this.dv.lissaldocs.items[0] = this.openitems[i];
                    } else {
                        this.dv.lissaldocs.items.push(this.openitems[i]);
                    }

...
export default {
    ...
    return {
      ...
      contextMenu: [
        ...
        { name: "separator" },
        {
          ...
          callback: async () => {
            this.selectedRowVar = this.dv.lissaldocs.items.findIndex(
              (e) => e._selected == true
            );
  ...
  methods: {
    async pushNewItem(event) {
      ...
      let myReturn = await this.lis.function("SALT01/pushNewItem", this.dv);

      myReturn.itemnum =
        this.dv.lissaldocs.items[this.dv.lissaldocs.items.length - 1].itemnum +
        10;

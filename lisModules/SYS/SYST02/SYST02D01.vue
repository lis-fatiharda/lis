      ...
      <l-div-flex>
        <l-card style="width: 50%; min-width: 320px">
          <div class="text-overline q-pa-xs">
            {{ this.$gl(`Sistem Parametreleri`, `System Parameters`) }}
          </div>
          <l-separator inset />
          <l-card-section-flex>
            <l-input
            ...
            />
            <l-input
              :label="this.$gl(`Kullanıcı Lisansı`, `User License`)"
          ...
          <l-card-section-flex>
            ...
            />
            <l-input
              :label="this.$gl(`Lis Socket Port`, `Lis Socket Port`)"
              v-model="dv.sys_lisSocketPort"
              readonly
            />
            <l-input
              :label="this.$gl(`Lis ApiOut Port`, `Lis ApiOut Port`)"
            ...
            <q-markup-table flat bordered dense>
              <thead>
                <tr class="bg-blue-grey-2">
                  <th class="text-left">{{ this.$gl(`Kullanıcı`, `User`) }}</th>
                  <th class="text-left">{{ this.$gl(`Token`, `Token`) }}</th>
                  <th class="text-left">
                    {{ this.$gl(`Login Date`, `Login Date`) }}
                  </th>
                  <th class="text-left">
                    {{ this.$gl(`Kullanıcı Konumu`, `User Location`) }}
                  </th>
                  <th class="text-left">
              ...
              <tbody>
                <tr v-for="item in dv.lisuserlogins" :key="item.id">
                  <td class="text-left">
                    {{ item.username }}
                  </td>
                  <td class="text-left">{{ item.token }}</td>
                  <td class="text-left">
                    <l-datetime v-model="item.logindate" :readonly="true" />
                  </td>
                  <td class="text-left">

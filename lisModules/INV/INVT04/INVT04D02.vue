  ...
  <l-div>
    ...
    <l-card>
      <l-toolbar :class="`bg-${tabInfo.moduleColor}`">
        ...
        <l-toolbar-title>{{
          this.$gl("Malzeme Hareketleri Detayı", "Material Movements Detail")
        }}</l-toolbar-title>
...
export default {
  ...
  methods: {
    async btnSave() {
      ...
      await this.lis.function("INVT04/02-btnSave", this.dv);
      this.cancel();
      // await this.$Axios
    ...
    cancel() {
      ...
      this.dv.lisDialog = "INVT04D01";
    },
    async init(prop) {
      await this.lis.function("INVT04/02-init", prop);
      // await this.$Axios

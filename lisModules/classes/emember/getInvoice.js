    ...
    var XMLBLOCK = await fs
        .readFileSync(
            path.join(
                ...
                "lisModules/classes/emember/wsXml/getInvoice.xml"
            )
        )
        .toString();
        ...
        .then((res) => {
            xml2js.parseString(res.data, async function (err, result) {
                ...
                for (let i in myReturn) {
                    ...
                    //--------------------------------------
                    olisedndocs.einvouuid = myParams.UUID;
                    olisedndocs.einvotype = myHeader["PROFILEID"][0];
                    olisedndocs.einvonumber = myParams.ID;
                    ...
                    Emember.markInvoice(pSession_id, myParams.UUID);
                }
            });
        })
        .catch(async (err) => {

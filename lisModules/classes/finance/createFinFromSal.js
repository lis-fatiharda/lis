...
export default async function (plissaldocs) { 
    ...
    const oliscustomers = (await liscustomers.findOne({ company: plissaldocs.company, customer: plissaldocs.customer, _deleted: false }))?.toObject()
    const olisaccounts = (await lisaccounts.findOne({ company: oliscustomers.company, acctype: oliscustomers.acctype, account: oliscustomers.account, _deleted: false }))?.toObject()
    if (olisaccounts == undefined) throw new Error("Cari Hesap Bulunamadı!")
    let olisfindocs = (new lisfindocs(lisfindocs.prototype.schema.tree)).toObject();
    const olissal001 = (await lissal001.findOne({ company: plissaldocs.company, doctype: plissaldocs.doctype }))?.toObject()
    ...
    for (let i in plissaldocs.items) {
        ...
        const olissal002 = (await lissal002.findOne({ company: plissaldocs.company, doctype: plissaldocs.doctype, itemtype: mySalItem.itemtype })).toObject();
        const olisfin002 = (await lisfin002.findOne({ company: olissal002.company, postkey: olissal002.postkey })).toObject();
        ...
        for (let k in olisfin002.items) {
            ...
            if (myfin002_item.acctype == "C") {
                ...
                if (olisfindocs.items.findIndex((x) => x.acctype == "C") > -1) {
                    let myRow = olisfindocs.items.findIndex((x) => x.acctype == "C");

                    if (myfin002_item.qpostway == false) {
                        olisfindocs.items[myRow].ddebit += mySalItem[myfin002_item.valuefield];
                    } else {
                        olisfindocs.items[myRow].dcredit += mySalItem[myfin002_item.valuefield];
                    }
                    ...
                    let olisfindocs_item = (new lisfindocs(lisfindocs.prototype.schema.tree)).toObject();
                    ...
                    if (myfin002_item.qpostway == false) {
                        olisfindocs_item.ddebit = mySalItem[myfin002_item.valuefield];
                    } else {
                        olisfindocs_item.dcredit = mySalItem[myfin002_item.valuefield];
                    }
            ...
            if (myfin002_item.acctype == "G") {
                ...
                let myAccount = await this.fetchAccount(mySalItem, myfin002_item.account, plissaldocs);

                if (olisfindocs.items.findIndex((x) => x.account == myAccount) > -1) {
                    let myRow = olisfindocs.items.findIndex((x) => x.account == myAccount);
                ...
                //
                    ...
                    let myAtext = await lisaccounts.findOne({ company: plissaldocs.company, acctype: olisfindocs_item.acctype, account: olisfindocs_item.account })

    ...
    var XMLBLOCK = await fs
        .readFileSync(
            path.join(
                ...
                "development/lisModules/classes/emember/wsXml/sendEarchive.xml"
            )
        )
        .toString();

    XMLBLOCK = XMLBLOCK.replace("#SESSION_ID#", pSession_id);
    XMLBLOCK = XMLBLOCK.replace("#ARCHIVETYPE#", "NORMAL");
            ...
            xml2js.parseString(res.data, async function (err, result) {
                ...
                if (
                    result["SOAP-ENV:Envelope"]["SOAP-ENV:Body"][0]
                        .ArchiveInvoiceExtendedResponse[0].ERROR_TYPE !=
                    undefined
                ) {
                    errorStr =
                        result["SOAP-ENV:Envelope"]["SOAP-ENV:Body"][0]
                            .ArchiveInvoiceExtendedResponse[0].ERROR_TYPE[0][
                            "ERROR_SHORT_DES"
                        ][0];
                }
